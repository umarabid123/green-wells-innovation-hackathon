...
// Helper to render professional icons by type
const NotificationIcon = ({ type }: { type: Notification["type"] }) => {
  const base = "h-5 w-5"
  switch (type) {
    case "traffic":
      return <MapPinIcon className={`${base} text-blue-600 dark:text-blue-400`} />
    case "route":
      return <RocketLaunchIcon className={`${base} text-emerald-600 dark:text-emerald-400`} />
    case "system":
      return <Cog6ToothIcon className={`${base} text-slate-600 dark:text-slate-300`} />
    case "alert":
      return <ExclamationTriangleIcon className={`${base} text-amber-600 dark:text-amber-400`} />
    default:
      return <BellIcon className={`${base} text-slate-500`} />
  }
}
...
export default function Navigation() {
  ...
  const { theme, setTheme } = useTheme() // Declare useTheme hook
  ...
  const addTestNotification = () => {
    const testNotifications: Array<Pick<Notification, "title" | "message" | "type">> = [
      {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <Link
            ...
            aria-label="TrafficWise AI Home"
          >
            <div className="h-8 w-8 rounded-md bg-blue-600 text-white grid place-items-center text-xs font-bold">
              TW
            </div>
            <span className="text-xl font-semibold text-slate-900 dark:text-white tracking-tight">TrafficWise AI</span>
          </Link>
                    ...
                    <div className="max-h-80 overflow-y-auto">
                      {notifications.length === 0 ? (
                        <div className="px-4 py-8 text-center">
                          <BellAlertIcon className="mx-auto h-8 w-8 text-slate-400" />
